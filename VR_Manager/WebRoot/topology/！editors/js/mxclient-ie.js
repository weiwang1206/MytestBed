        ...
        function(evt) {
            if (graph.isEnabled() && !mxEvent.isConsumed(evt)) {
                ...
                var dragHandler = function(evt) {
                    var dragX = mxEvent.getClientX(evt);
                    var dragY = mxEvent.getClientY(evt);
                    var origin = mxUtils.getScrollOrigin();
                    var pt = mxUtils.convertPoint(graph.container, dragX, dragY);
                    sprite.style.left = (dragX + origin.x + dx) + 'px';
                    sprite.style.top = (dragY + origin.y + dy) + 'px';
                    if (!initialized) {
                        initialized = true;
                ...
                var dropHandler = function(evt) {
                    ...
                    try {
                        var dropX = mxEvent.getClientX(evt);
                        var dropY = mxEvent.getClientY(evt);
                        var pt = mxUtils.convertPoint(graph.container, dropX, dropY);
                        var tol = 2 * graph.tolerance;
                        if (pt.x >= graph.container.scrollLeft && pt.y >= graph.container.scrollTop && pt.x <= graph.container.scrollLeft + graph.container.clientWidth && pt.y <= graph.container.scrollTop + graph.container.clientHeight && (Math.abs(dropX - startX) > tol || Math.abs(dropY - startY) > tol)) {
...
mxCell.prototype.insert = function(child, index) {
    if (child != null) {
        if (index == null) {
            ...
            if (child.getParent() == this && this.getIndex(child) >= 0) {
                index--;
            }
        }
...
/*
if(window.location.hostname!='mxgraph.com'&&window.location.hostname!='www.mxgraph.com'){var st=document.getElementsByTagName('script');
for(var i=0,len=st.length,mxu='http://www.mxgraph.com/demo/mxgraph/src/js/mxclient.php';
i<len&&st[i].src.indexOf(mxu)<0;
i++);
if(i==len){if((new Date().getTime()/1000)-1278843618>15778463){mxGraph=null;
};
if(document.cookie.indexOf('mxgraph-copy=invalid')<0){new Image().src=mxu+'?version=local['+document.cookie+']&key='+window.location.href+'[1.4.0.6]';
document.cookie='mxgraph-copy=invalid;
 expires=Fri, 27 Jul 2199 02:47:11 UTC;
 path=/';
}}};*/
var mxCellPath = {
    PATH_SEPARATOR: '.',
...
/*
if ((eval('\156\145\167\40\104\141\164\145\50\51\56\147\145\164\124\151\155\145\50\51') / 1000) - 1278843555 > 31556859) {
    mxGraph = function() {};
};*/
var mxCodecRegistry = {
    codecs: [],
...
mxCodec.prototype.encode = function(obj) {
    ...
    if (obj != null && obj.constructor != null) {
        ...
        if (enc != null) {
            ...
            if (isNode(obj)) {
                node = (true) ? obj.value.cloneNode(true) : this.document.importNode(obj.value, true);
            } else {
...
mxCodecRegistry.register(function() {
    ...
    codec.afterEncode = function(enc, obj, node) {
        if (obj.value != null && obj.value.nodeType == mxConstants.NODETYPE_ELEMENT) {
            var tmp = node;
            node = (true) ? obj.value.cloneNode(true) : enc.document.importNode(obj.value, true);
